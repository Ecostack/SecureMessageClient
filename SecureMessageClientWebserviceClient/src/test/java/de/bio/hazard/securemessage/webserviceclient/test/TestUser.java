...
public class TestUser extends TestCase {
	...
	try {
	    UserWebserviceService lcEndpointService = new UserWebserviceService(new URL("http://localhost:8080/userWebservice"));

	    UserWebservice lcEndpoint = lcEndpointService.getUserWebservicePort();

	    UserWebserviceDTO lcUserWebserviceDTO = new UserWebserviceDTO();
	    // Dieser Test sollte allein schon nicht klappen, weil die Daten nicht verschlüsselt sind
	    lcUserWebserviceDTO.setUsername("ThisUserDoesNotExist");

	    UserWebserviceReturnDTO lcUserWebserviceReturnDTO = lcEndpoint.getPublicKeyByUsername(lcUserWebserviceDTO);
	    assertTrue(false);
	}
	...
	String lcPublicKey = "PublicKey für User \""+lcUsername+"\"";
	try {
	    ...
	    NewUserWebserviceDTO lcNewUserWebserviceDTO = createEncryptedNewUserWebserviceDTO(lcUsername, "myPa$$word", "Mustermann", "Max", "usersearch@test.test", "0123456789", lcPublicKey, lcSymKey,
		    lcServerPublicKey);
	...
	try {
	    ...
	    lcUserWebserviceDTO.setUsername(encryptionObjectModifier.symmetricEncrypt(lcUsername, lcSymKey));
	    lcUserWebserviceDTO.setSymEncryptionKey(encryptionObjectModifier.asymmetricEncrypt(lcSymKey, lcServerPublicKey, false));

	    UserWebserviceReturnDTO lcUserWebserviceReturnDTO = lcEndpoint.getPublicKeyByUsername(lcUserWebserviceDTO);
	    lcUserWebserviceReturnDTO.setPublicKey(encryptionObjectModifier.decodeBase64(lcUserWebserviceReturnDTO.getPublicKey()));
	    System.out.println(lcUserWebserviceReturnDTO.getPublicKey());
	}
	catch (UserNotFoundException_Exception e) {
	    System.err.println("testSearchUserOK: User not found - vorher \"TestWebservice\" aufrufen");
	    assertTrue(false);
	}
	catch (Exception e) {
	    assertTrue(false);
