...
public class MessageServiceFacade {
	...
	public void sendMessage(Message pMessage,
			HashMap<String, byte[]> pUserPublicKeys,
			CommunicationKeyHelper pMessageKey)
			throws EncryptionExceptionBiohazard {
		MessageWebserviceDTO lcMessageWebserviceDTO = transformMessageToDTO(
				pMessage, pUserPublicKeys);
		lcMessageWebserviceDTO = encryptMessageWebserviceDTO(
				lcMessageWebserviceDTO, pMessageKey);
		try {
	...
	public List<Message> getMessages(CommunicationKeyHelper pCommunicationKey)
		...
		try {
			RequestMessageWebserviceReturnDTO lcRequestMessageWebserviceReturnDTO = getMessageWSPort()
					.getMessages(lcRequestMessageWebserviceDTO);
			lcRequestMessageWebserviceReturnDTO = decryptRequestMessageWebserviceReturnDTO(
					lcRequestMessageWebserviceReturnDTO, pCommunicationKey);
			lcReturn = transformRequestMessageReturnDTOToMessagelist(
					lcRequestMessageWebserviceReturnDTO, pCommunicationKey);
	...
	private RequestMessageWebserviceReturnDTO decryptRequestMessageWebserviceReturnDTO(
			RequestMessageWebserviceReturnDTO pRequestMessageWebserviceReturnDTO,
		...
		try {
			...
			byte[] lcSymmetricKey = encryptionObjectModifier
					.asymmetricDecryptToByte(
							lcRequestMessageWebserviceReturnDTO
									.getSymEncryptionKey(), pCommunicationKey
			...
			for (MessageWebserviceDTO lcMessage : lcRequestMessageWebserviceReturnDTO
				...
				for (MessageContentWebserviceDTO lcMessageContent : lcMessage
					...
					lcMessageContent.setData(encryptionObjectModifier
							.symmetricDecrypt(lcMessageContent.getData(),
									lcSymmetricKey));
					lcMessageContent.setFilename(encryptionObjectModifier
							.symmetricDecrypt(lcMessageContent.getFilename(),
									lcSymmetricKey));
					for (MessageContentKeyWebserviceDTO lcMessageContentKey : lcMessageContent
						...
						lcMessageContentKey
								.setSymmetricEncryptionKey(encryptionObjectModifier
										.symmetricDecrypt(lcMessageContentKey
	...
	private RequestMessageWebserviceDTO encryptRequestMessageWebserviceDTO(
			CommunicationKeyHelper pCommunicationKey)
			throws EncryptionExceptionBiohazard {
	...
	private MessageWebserviceDTO encryptMessageWebserviceDTO(
		...
		try {
			byte[] lcSymmetricKey = symmetricKeygen.getKey(128);
			lcMessageWebserviceDTO.setSymEncryptionKey(encryptionObjectModifier
					.asymmetricEncrypt(lcSymmetricKey,
							pMessageKey.getServerPublicKey(), false));
			lcMessageWebserviceDTO
					.setTokenId(encryptionObjectModifier.symmetricEncrypt(
							pMessageKey.getTokenId(), lcSymmetricKey));
			for (MessageContentWebserviceDTO mcwDTO : lcMessageWebserviceDTO
					.getContent()) {
				mcwDTO.setData(encryptionObjectModifier.symmetricEncrypt(
						mcwDTO.getData(), lcSymmetricKey));
				mcwDTO.setFilename(encryptionObjectModifier.symmetricEncrypt(
				...
				for (MessageContentKeyWebserviceDTO mckwDTO : mcwDTO
					...
					mckwDTO.setUsername(encryptionObjectModifier
							.symmetricEncrypt(mckwDTO.getUsername(),
									lcSymmetricKey));
	...
	private MessageWebserviceDTO transformMessageToDTO(Message pMessage,
			HashMap<String, byte[]> pUserPublicKeys)
			throws EncryptionExceptionBiohazard {
		...
		try {
			for (MessageReceiver lcReceiver : pMessage.getReceiver()) {
				...
				lcMessageReceiverDTO.setMessageReceiverType(lcReceiver
						.getMessageReceiverType().toString());
				lcMessageWebserviceDTO.getReceiver().add(lcMessageReceiverDTO);
			}
			for (MessageContent lcContent : pMessage.getContent()) {
				MessageContentWebserviceDTO lcMessageContentDTO = transformMessageContentToDTO(
						lcContent, pUserPublicKeys);
	...
	private List<Message> transformRequestMessageReturnDTOToMessagelist(
			RequestMessageWebserviceReturnDTO pRequestMessageWebserviceReturnDTO,
			CommunicationKeyHelper pCommunicationKey)
			throws EncryptionExceptionBiohazard {
		List<Message> lcReturn = new ArrayList<Message>();
		try {
			for (MessageWebserviceDTO lcMessageDTO : pRequestMessageWebserviceReturnDTO
					.getMessages()) {
				Message lcMessage = new Message();
				// TODO Receiver einbauen (wenn serverseitig implementiert)
				for (MessageContentWebserviceDTO lcContentDTO : lcMessageDTO
						.getContent()) {
					MessageContent lcContent = transformDTOToMessageContent(
							lcContentDTO, pCommunicationKey);
	...
	private MessageContent transformDTOToMessageContent(
			MessageContentWebserviceDTO pContentDTO,
			CommunicationKeyHelper pCommunicationKey) throws IOException,
			InvalidKeyException, IllegalBlockSizeException, BadPaddingException {
		MessageContent lcMessageContent = new MessageContent();
		byte[] lcSymmetricKey = encryptionObjectModifier
				.asymmetricDecryptToByte(pContentDTO.getSymmetricKeys().get(0)
						.getSymmetricEncryptionKey(),
						pCommunicationKey.getUserPrivateKey(), true);
		lcMessageContent.setData(encryptionObjectModifier
				.symmetricDecryptToByte(pContentDTO.getData(), lcSymmetricKey));
		lcMessageContent.setFilename(encryptionObjectModifier.symmetricDecrypt(
				pContentDTO.getData(), lcSymmetricKey));
		return lcMessageContent;
	}

	private MessageContentWebserviceDTO transformMessageContentToDTO(
			MessageContent lcContent, HashMap<String, byte[]> pUserPublicKeys)
			throws NoSuchAlgorithmException, NoSuchProviderException,
			InvalidKeyException, IllegalBlockSizeException,
			BadPaddingException, IOException {
		MessageContentWebserviceDTO lcMessageContentDTO = new MessageContentWebserviceDTO();
		byte[] lcSymmetricKey = symmetricKeygen.getKey(128);
		lcMessageContentDTO.setData(encryptionObjectModifier.symmetricEncrypt(
				lcContent.getData(), lcSymmetricKey));
		lcMessageContentDTO.setFilename(encryptionObjectModifier
				.symmetricEncrypt(lcContent.getFilename(), lcSymmetricKey));

		for (String lcContentReceiver : pUserPublicKeys.keySet()) {
			MessageContentKeyWebserviceDTO lcMessageContentKeyWebserviceDTO = buildMessageContentKeyWebserviceDTO(
					pUserPublicKeys, lcContentReceiver, lcSymmetricKey);
			lcMessageContentDTO.getSymmetricKeys().add(
					lcMessageContentKeyWebserviceDTO);
		}
		return lcMessageContentDTO;
	}

	private MessageContentKeyWebserviceDTO buildMessageContentKeyWebserviceDTO(
			HashMap<String, byte[]> pUserPublicKeys, String lcContentReceiver,
			byte[] lcSymmetricKey) throws IOException {
		...
		lcMessageContentKeyWebserviceDTO
				.setSymmetricEncryptionKey(encryptionObjectModifier
						.asymmetricEncrypt(lcSymmetricKey,
								pUserPublicKeys.get(lcContentReceiver), false));
